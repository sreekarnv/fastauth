...
def test_create_refresh_token(session: Session):
    ...
    token = adapter.create(
        user_id=user.id, token_hash="token_hash_123", expires_at=expires_at
    )
...
def test_get_active_excludes_revoked(session: Session):
    ...
    adapter.create(user_id=user.id, token_hash="revoked_token", expires_at=expires_at)
...
def test_revoke_token(session: Session):
    ...
    token = adapter.create(
        user_id=user.id, token_hash="token_to_revoke", expires_at=expires_at
    )
...
def test_revoke_already_revoked_token(session: Session):
    ...
    adapter.create(user_id=user.id, token_hash="already_revoked", expires_at=expires_at)
